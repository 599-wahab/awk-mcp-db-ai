...
interface Message {
  ...
  showTable?: boolean; // user-controlled table toggle (hidden by default)
}

interface App { id: string; name: string; apiKey: string; }

interface HistoryLog {
  id: string; question: string; explanation: string | null;
  generatedSql: string | null; result: string | null;
  feedback: string | null; detectedLang: string | null;
  createdAt: string; appId: string;
}
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {["QUOTA_EXCEEDED","NO_KEY","INVALID_KEY","MODEL_NOT_FOUND"].includes(errorType || "") && (
            <a href="/dashboard/settings" className="text-[10px] underline" style={{ color: "#e8ff47" }}>→ Fix in Settings</a>
          )}
        </div>
        <button onClick={onClose} className="text-[#3a3a3a] hover:text-white shrink-0">✕</button>
      </div>
    </div>
  );
}
...
function MicOverlay({ onStop, isUr }: { onStop: () => void; isUr: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/92 backdrop-blur-sm" onClick={onStop}>
      <style>{`
        @keyframes ripple { 0%{transform:scale(1);opacity:.6} 100%{transform:scale(2.8);opacity:0} }
        .rp1{animation:ripple 1.6s ease-out infinite}
        .rp2{animation:ripple 1.6s ease-out .55s infinite}
        .rp3{animation:ripple 1.6s ease-out 1.1s infinite}
      `}</style>
      <div className="relative flex items-center justify-center w-36 h-36 mb-8">
        <div className="rp1 absolute w-24 h-24 rounded-full border-2" style={{borderColor:"#e8ff47"}}/>
        <div className="rp2 absolute w-24 h-24 rounded-full border-2" style={{borderColor:"#e8ff47"}}/>
        <div className="rp3 absolute w-24 h-24 rounded-full border-2" style={{borderColor:"#e8ff47"}}/>
        <div className="relative w-24 h-24 rounded-full flex items-center justify-center" style={{background:"#e8ff47"}}>
          <svg className="w-11 h-11 text-black" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/>
            <path d="M19 10v2a7 7 0 0 1-14 0v-2H3v2a9 9 0 0 0 8 8.94V22h-3v2h8v-2h-3v-1.06A9 9 0 0 0 21 12v-2h-2z"/>
          </svg>
        </div>
      </div>
      <p className="fd text-4xl tracking-widest mb-3" style={{color:"#e8ff47"}}>
        {isUr ? "بول رہے ہیں..." : "LISTENING..."}
      </p>
      <p className="fm text-xs text-[#5a5a5a]">{isUr ? "بند کرنے کے لیے ٹیپ کریں" : "Tap anywhere to stop"}</p>
    </div>
  );
}

// ── Clarification bubble ──────────────────────────────────────────────────────
function ClarificationBubble({ message, isUr, onReply }: {
  message: string; isUr: boolean; onReply: (t: string) => void;
}) {
  const [val, setVal] = useState("");
  return (
    <div className="bg-black border border-[#e8ff47]/40 px-4 py-3 space-y-3 w-full sm:max-w-[85%]">
      <p className="fm text-[9px] text-[#e8ff47] uppercase tracking-wider">// {isUr ? "وضاحت درکار ہے" : "Need clarification"}</p>
      <p className="text-sm leading-relaxed" dir="auto">{message}</p>
      <div className="flex gap-2">
        <input
          value={val} onChange={e => setVal(e.target.value)}
          onKeyDown={e => e.key === "Enter" && val.trim() && (onReply(val), setVal(""))}
          placeholder={isUr ? "یہاں جواب دیں..." : "Your answer..."}
          dir="auto"
          className="flex-1 bg-[#0d0d0d] border border-[#1e1e1e] text-white px-3 py-2 text-xs focus:outline-none focus:border-[#e8ff47] transition-colors"
          style={{fontSize:"16px"}}
        />
        <button
          onClick={() => { if (val.trim()) { onReply(val); setVal(""); } }}
          className="fd text-sm px-4 py-2 text-black shrink-0"
          style={{background:"#e8ff47"}}
        >{isUr ? "بھیجیں" : "SEND"}</button>
      </div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  // Build chat history context to send to backend
  const getChatContext = () =>
    messages.slice(-8).map(m => ({ isUser: m.isUser, content: m.content }));

  function restoreChat(log: HistoryLog) {
    setMessages([
      { id:`h-${log.id}-q`, content:log.question, isUser:true, timestamp:new Date(log.createdAt) },
      {
        id:`h-${log.id}-a`, content:log.explanation||(isUr?"پرانا نتیجہ":"Past result"),
        isUser:false, timestamp:new Date(log.createdAt),
        sql:log.generatedSql||undefined,
        result:log.result ? (() => { try { return JSON.parse(log.result!); } catch { return []; } })() : undefined,
        visualization:"table", showTable:false, // restored chats default to table-hidden
        detectedLang:(log.detectedLang as "ur"|"en")||undefined,
        chatLogId:log.id, feedback:(log.feedback as "like"|"dislike"|null)||null,
      },
    ]);
  ...
  async function sendFeedback(chatLogId: string, feedback: "like"|"dislike") {
    ...
    setMessages(prev => prev.map(m =>
      m.chatLogId === chatLogId ? {...m, feedback: m.feedback===feedback ? null : feedback} : m
    ));
  }

  function toggleTable(msgId: string) {
    setMessages(prev => prev.map(m => m.id===msgId ? {...m, showTable:!m.showTable} : m));
  }
  ...
  const ask = useCallback(async (text: string) => {
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body:JSON.stringify({ question:text, preferredLang, chatHistory:getChatContext() }),
      });
      ...
      if (!res.ok || data.error) {
        ...
        setMessages(m => [...m, {
          id:(Date.now()+1).toString(), content:data.error||"An error occurred.",
          isUser:false, timestamp:new Date(), isError:true, errorType:data.errorType,
        }]);
      } else if (data.isClarification) {
        // Bot is asking a follow-up question
        setMessages(m => [...m, {
        ...
        const isTableViz = data.visualization === "table";
        setMessages(m => [...m, {
          ...
          feedback:null,
          showTable:false, // always hidden by default — user must click to reveal
        }]);
        if (ttsOn && data.explanation) speak(data.explanation, preferredLang);
      }
    } catch {
    ...
    setLoading(false);
  }, [selectedKey, preferredLang, ttsOn, isUr, speak, messages]);

  function handleMic() {
    if (recording) { stopRef.current?.(); setRecording(false); return; }
    setRecording(true);
    stopRef.current = listen(t => { setRecording(false); ask(t); }, ()=>setRecording(false), preferredLang);
  }
  ...
  return (
    <div className={`flex h-full overflow-hidden ${msgSize}`} style={{fontFamily:"'DM Sans',sans-serif"}}>
      <style>{`
      ...
      {/* ── History sidebar ─────────────────────────────────────────────── */}
      {showHistory && (
        <div className="fixed inset-0 z-40 flex" onClick={e=>{if(e.target===e.currentTarget)setShowHistory(false);}}>
          <div className="absolute inset-0 bg-black/70" onClick={()=>setShowHistory(false)}/>
          <aside className="relative z-10 w-72 max-w-[85vw] h-full border-r border-[#1e1e1e] bg-[#060606] flex flex-col">
            ...
            <button
              ...
              className="mx-3 mt-3 mb-1 fd text-sm tracking-wider py-2 text-black hover:opacity-90 transition"
              style={{background:"#e8ff47"}}
            >+ {isUr?"نئی گفتگو":"NEW CHAT"}</button>
              ...
              {historyLoading
                ...
                : history.length===0
                  ...
                  : Object.entries(grouped).map(([date, logs])=>(
                    <div key={date}>
                      <p className="fm text-[9px] text-[#3a3a3a] uppercase tracking-wider px-4 py-2 sticky top-0 bg-[#060606]">{date}</p>
                      {logs.map(log=>(
                        <button key={log.id} onClick={()=>restoreChat(log)}
                          className="w-full text-left px-4 py-2.5 hover:bg-[#111] transition-colors group">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {log.detectedLang && <span className="fm text-[8px] text-[#3a3a3a]">[{log.detectedLang}]</span>}
                          </div>
                        </button>
                      ))}
                    </div>
      ...
      <div className="flex flex-col flex-1 min-w-0 border border-[#1e1e1e] bg-[#0d0d0d] overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[#1e1e1e] shrink-0">
          <button onClick={()=>setShowHistory(!showHistory)}
            ...
            title={isUr?"پرانی گفتگو":"Chat History"}>☰</button>
          {apps.length>0 ? (
            <select value={selectedKey} onChange={e=>setSelectedKey(e.target.value)}
              className="fm text-xs bg-black border border-[#1e1e1e] text-white px-2 py-1.5 focus:outline-none focus:border-[#e8ff47] transition-colors flex-1 min-w-0 max-w-[180px] truncate">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
          {messages.length===0 && (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4 px-4 py-8">
              ...
              <p className="text-[#5a5a5a] text-sm" dir="auto">{isUr?"اردو یا English میں سوال پوچھیں":"Ask in Urdu or English"}</p>
          ...
          {messages.map(msg => {
            ...
            const isTableType = msg.visualization === "table";

            return (
              <div key={msg.id} className={`flex ${msg.isUser?"justify-end":"justify-start"}`}>
                <div className={`
                  ${msg.isUser?"max-w-[85%] sm:max-w-[70%] text-black":"w-full sm:max-w-[92%] text-white"}
                  ${!msg.isUser && msg.isError?"bg-red-500/10 border border-red-500/30":""}
                  ${!msg.isUser && !msg.isError && !msg.isClarification?"bg-black border border-[#1e1e1e]":""}
                  ${msg.isClarification?"":""}
                `} style={msg.isUser?{background:"#e8ff47"}:{}}>
                  <div className={`px-3 py-2.5 ${msg.isClarification?"p-0":""}`}>
                    {/* Label */}
                    {!msg.isClarification && (
                      <p className={`fm text-[9px] mb-1.5 ${msg.isUser?"text-black/50":msg.isError?"text-red-400":"text-[#3a3a3a]"}`}>
                        {msg.isUser?(isUr?"آپ":"YOU"):msg.isError?"ERROR":"AWKT-LD BOT"}
                    ...
                    {/* Clarification */}
                    {msg.isClarification && (
                      <ClarificationBubble message={msg.content} isUr={isUr} onReply={ask}/>
                    )}

                    {/* Normal message */}
                    {!msg.isClarification && (
                      <p className={`whitespace-pre-wrap leading-relaxed ${msgSize}`} dir="auto">{msg.content}</p>
                    )}
                    ...
                    {/* KPI */}
                    {!msg.isUser && !msg.isError && !msg.isClarification && msg.visualization==="kpi" && msg.result?.[0] && (
                    ...
                    {/* Chart */}
                    {!msg.isUser && !msg.isError && !msg.isClarification && isChartable && (
                      <div className="mt-3">
                    ...
                    {/* TABLE — hidden by default, revealed by user click */}
                    {!msg.isUser && !msg.isError && !msg.isClarification && hasResult && (
                      <div className="mt-2.5">
                        ...
                        >
                          {msg.showTable
                            ? (isUr?"▲ ٹیبل چھپائیں":"▲ Hide table")
                            : (isUr?`▼ ٹیبل دیکھیں (${msg.result!.length} rows)`:`▼ Show table (${msg.result!.length} rows)`)}
                        </button>
                        {msg.showTable && (
                    ...
                    {/* Insights */}
                    {!msg.isUser && !msg.isError && !msg.isClarification && msg.insights?.length ? (
                    ...
                    {/* SQL */}
                    {!msg.isUser && !msg.isError && !msg.isClarification && msg.sql && (
                      <details className="mt-2">
                        <summary className="fm text-[10px] text-[#3a3a3a] cursor-pointer hover:text-[#5a5a5a] select-none">
                          {isUr?"// SQL دیکھیں":"// View SQL"}
                        </summary>
                        <pre className="bg-black border border-[#1e1e1e] text-[#e8ff47] fm text-[10px] p-2 mt-1 overflow-x-auto whitespace-pre-wrap break-all max-h-48">{msg.sql}</pre>
                      </details>
                    )}

                    {/* Like/Dislike */}
                    {!msg.isUser && !msg.isError && !msg.isClarification && msg.chatLogId && (
                      <div className="flex items-center gap-2 mt-2.5 pt-2 border-t border-[#1e1e1e]">
                        <span className="fm text-[9px] text-[#3a3a3a]">{isUr?"کیسا جواب؟":"Helpful?"}</span>
                        <button onClick={()=>sendFeedback(msg.chatLogId!,"like")}
                          className={`fm text-[10px] px-2 py-1 border transition-colors ${
                            msg.feedback==="like"?"border-green-500 text-green-400 bg-green-500/10":"border-[#1e1e1e] text-[#5a5a5a] hover:border-green-500 hover:text-green-400"
                          }`}>👍</button>
                        <button onClick={()=>sendFeedback(msg.chatLogId!,"dislike")}
                          className={`fm text-[10px] px-2 py-1 border transition-colors ${
                            msg.feedback==="dislike"?"border-red-500 text-red-400 bg-red-500/10":"border-[#1e1e1e] text-[#5a5a5a] hover:border-red-500 hover:text-red-400"
                          }`}>👎</button>
                        {msg.feedback && (
                          <span className="fm text-[9px] text-[#5a5a5a]">
                            {msg.feedback==="like"?(isUr?"پسند کیا":"Liked"):(isUr?"ناپسند کیا":"Disliked")}
                          </span>
                        )}
                      </div>
                    )}
        ...
        {/* Input */}
        <div className="px-3 pt-2 border-t border-[#1e1e1e] shrink-0"
          ...
          <div className="flex gap-2 items-center">
            <input ref={inputRef} value={question}
              ...
              dir="auto" disabled={loading}
              className="flex-1 min-w-0 bg-black border border-[#1e1e1e] text-white px-3 py-2.5 focus:outline-none focus:border-[#e8ff47] transition-colors disabled:opacity-50"
              style={{fontSize:"16px"}}/>
