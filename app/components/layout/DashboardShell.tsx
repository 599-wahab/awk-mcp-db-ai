...
export default function DashboardShell({
  ...
  useEffect(() => {
    fetch("/api/user-preferences")
      .then((r) => r.json())
      .then((d) => {
        if (d.preferredLang) setLang(d.preferredLang);
      })
      .catch(() => {});
    ...
    >
      ...
      <header className="border-b border-[#1e1e1e] bg-black px-4 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <button
            className="md:hidden text-[#5a5a5a]"
          ...
          <Link href="/" className="fd text-xl tracking-wider">
            ...
            <span
              className="fm text-[10px] border border-[#e8ff47] ml-1.5 px-1.5 py-0.5 align-middle"
              style={{ color: "#e8ff47" }}
            >
              BOT
            </span>
        ...
        <div className="flex items-center gap-2 fm text-[10px]">
          ...
          <span className="text-[#3a3a3a] hidden sm:block truncate max-w-25">
