...
const NAV = [
  { icon: "💬", labelEn: "AI Chat",        labelUr: "اے آئی چیٹ",  href: "/dashboard" },
  { icon: "🔌", labelEn: "Connected Apps", labelUr: "ایپس",         href: "/dashboard/widget-sites" },
  { icon: "⚙️", labelEn: "Settings",       labelUr: "ترتیبات",      href: "/dashboard/settings" },
];
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        `}>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 text-sm transition-colors ${
                    active ? "border-l-2 pl-2.5 text-[#e8ff47]" : "text-[#5a5a5a] hover:text-white"
                  }`}
                  style={active ? { borderColor: "#e8ff47" } : {}}
                  dir={isUr ? "rtl" : "ltr"}
                >
                  <span>{item.icon}</span>
                  <span className={active ? "font-medium" : ""}>{isUr ? item.labelUr : item.labelEn}</span>
                </Link>
              );
            })}
          </nav>
          ...
          <div className="mt-auto border border-[#e8ff47]/20 bg-[#e8ff47]/5 p-3">
            ...
            <p className="text-xs text-[#5a5a5a] leading-relaxed" dir="rtl">
              مائیک بٹن دبا کر اردو میں بولیں
            </p>
