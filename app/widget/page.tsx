...
function WidgetChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([{
    id: "w", content: "السلام علیکم! اردو یا English میں سوال پوچھیں۔\nHello! Ask me anything about your data.",
  ...
  return (
    <div className="h-screen flex flex-col bg-black text-white overflow-hidden" style={{ fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {/* Header */}
      <div className="px-4 py-3 flex items-center gap-3 shrink-0 border-b border-[#1e1e1e]" style={{ background: "#e8ff47" }}>
        <div className="fd text-xl tracking-wider text-black">AWK<span className="opacity-60"> TLD</span></div>
        <div className="flex-1">
          <p className="fm text-[10px] text-black/60 uppercase tracking-wider">BOT • اردو / English</p>
        </div>
        <button onClick={() => setTtsOn(v => !v)} className="fm text-[10px] bg-black/10 text-black px-2 py-1 hover:bg-black/20 transition-colors">
          {ttsOn ? "🔊" : "🔇"}
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.isUser ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[90%] px-3 py-2.5 ${msg.isUser ? "text-black" : "bg-[#0d0d0d] border border-[#1e1e1e] text-white"}`}
              style={msg.isUser ? { background: "#e8ff47", borderRadius: 0 } : { borderRadius: 0 }}>
              <p className={`fm text-[9px] mb-1 ${msg.isUser ? "text-black/50" : "text-[#3a3a3a]"}`}>
                {msg.isUser ? "YOU" : "BOT"} • {new Date(msg.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              </p>
              ...
              {!msg.isUser && msg.visualization === "kpi" && msg.result?.length === 1 && (
                <div className="fd text-3xl mt-2" style={{ color: "#e8ff47" }}>{String(Object.values(msg.result[0])[0])}</div>
              )}
              ...
              {!msg.isUser && msg.visualization === "table" && msg.result && (
                <div className="overflow-x-auto mt-2">
                  <table className="min-w-full border border-[#1e1e1e] fm text-[10px]">
                    <thead><tr className="border-b border-[#1e1e1e]">{Object.keys(msg.result[0]).map(c => <th key={c} className="px-2 py-1 text-left text-[#5a5a5a] font-normal">{c}</th>)}</tr></thead>
                    <tbody>{msg.result.map((row, i) => <tr key={i} className="border-b border-[#0a0a0a]">{Object.values(row).map((v, j) => <td key={j} className="px-2 py-1 text-[#5a5a5a]">{String(v)}</td>)}</tr>)}</tbody>
                  </table>
                </div>
              )}
              ...
              {!msg.isUser && msg.insights?.length ? (
                <div className="mt-2 space-y-0.5 border-t border-[#1e1e1e] pt-2">
                  {msg.insights.map((ins, i) => <p key={i} className="fm text-[9px] text-[#e8ff47]">▸ {ins}</p>)}
                </div>
              ) : null}

              {!msg.isUser && msg.sql && (
                <details className="mt-2">
                  <summary className="fm text-[9px] text-[#3a3a3a] cursor-pointer">// SQL</summary>
                  <pre className="bg-black border border-[#1e1e1e] text-[#e8ff47] fm text-[9px] p-2 mt-1 overflow-x-auto">{msg.sql}</pre>
                </details>
              )}
            </div>
          </div>
      ...
      <div className="px-3 pb-3 pt-2 border-t border-[#1e1e1e] shrink-0">
        <div className="flex gap-1.5 items-center">
          <input value={question} onChange={e => setQuestion(e.target.value)}
            ...
            placeholder="اردو یا English..." dir="auto"
...
export default function WidgetPage() {
  return (
    <Suspense fallback={<div className="h-screen flex items-center justify-center bg-black"><div className="fd text-2xl text-[#e8ff47]">LOADING...</div></div>}>
      <WidgetChat />
    </Suspense>
  );
}