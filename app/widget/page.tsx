    ...
    <div className="h-screen flex flex-col bg-white overflow-hidden">
      ...
      <div className="bg-indigo-600 px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-white font-semibold text-sm leading-none">AWKT-LD BOT</p>
          ...
          <div key={msg.id} className={`flex ${msg.isUser ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[90%] rounded-2xl px-3 py-2.5 ${
              ...
              <p className="text-xs opacity-50 mb-1">
                {msg.isUser ? "You" : "AWKT-LD"} • {new Date(msg.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              </p>
                    ...
                    {["line", "bar", "stacked", "pie"].map(t => (
                      <button
                        ...
                        className={`text-xs px-2 py-1 rounded-lg transition-colors ${
                          getViewType(msg.id, msg.visualization!) === t
                    ...
                    <thead className="bg-gray-200">
                      <tr>
                        {Object.keys(msg.result[0]).map(c => (
                          <th key={c} className="border-b px-2 py-1 text-left text-gray-700">{c}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
              ...
              {!msg.isUser && msg.insights?.length ? (
                <div className="mt-2 space-y-1">
                  {msg.insights.map((ins, i) => (
                    <p key={i} className="text-xs text-indigo-600">📌 {ins}</p>
                  ))}
                </div>
              ) : null}
